...
#
# -------------------- VERSION 1.5.0 ---------------- VERSION 1.5.0 ---------------- VERSION 1.5.0 ------------------
#
# ---------------------------------------------- ~ - ~ Ω <(-.-)> Ω ~ - ~ --------------------------------------------
...
def reverse_dns_lookup(ip, local_dns_servers): # Looks for the Local DNS Server for host name resolution
    ...
    for dns_server in local_dns_servers:
            try:
                result = subprocess.run(["nslookup", ip, dns_server],
...
def Public_Grabber(): # Grabs the public IP address of teh current network
